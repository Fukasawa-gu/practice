...
id_list = ["taro", "jiro", "saburo"]
for i in id_list:
    #ログイン
    print("{}でログイン".format(i))
    name = i
    ...
    if i == "taro":
        input.send_keys('bigbrother')
        time.sleep(1)
    elif i == "jiro":
        input.send_keys('middlebrother')
        time.sleep(1)
    else:
        input.send_keys('littlebrother')
        time.sleep(1)
    ...
    other_seller = id_list
    other_seller.remove(i)
    print(other_seller)

    # ログインしているアカウント以外を一つずつ見ていく
    ...
    print('{}は他のアカウントを全て見終わったのでログアウトします。'.format(name))
    chro.get('https://auctions.yahoo.co.jp/')
    time.sleep(1)
    chro.find_element_by_link_text('ログアウト').click()
    time.sleep(1)

print("すべてのアカウントで相互にウォッチリスト登録終了しました。")